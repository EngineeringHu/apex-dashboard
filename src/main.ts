...
export default class DashboardPlugin extends Plugin {
	...
	async onload(): Promise<void> {
		...
		this.registerEvent(this.app.workspace.on('file-open', async (file: TFile | null) => {
			...
			if (activeLeaf?.view instanceof DashboardView) return;
	...
	/** Switch to a workspace and render it in the given leaf (or a new dashboard
	 *  leaf if none is suitable). */
	private async renderWorkspaceInLeaf(path: string, leaf: WorkspaceLeaf | null): Promise<void> {
		await this.switchWorkspace(path);
		if (leaf && (leaf.view as any).getViewType?.() === 'markdown') {
