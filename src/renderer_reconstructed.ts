import { App, Menu, Modal, Platform, TFile, setIcon } from 'obsidian';
...
export function renderSection(column: DashboardColumn, callbacks: RenderCallbacks, app: App, data?: DashboardData, settings?: DashboardSettings): HTMLElement {
	...
	// Table section (type: 'table'): render the raw markdown table stored in the
	// dashboard file body, honoring Table Extended style `>` (colspan) and
	// `^` (rowspan) merge markers.
	if (sectionType === 'table') {
		const editTableBtn = headerActions.createEl('button', {
			cls: 'dashboard-section-add-btn',
			attr: { 'aria-label': t('table.edit') },
		});
		setIcon(editTableBtn, 'pencil');
		editTableBtn.addEventListener('click', () => {
			new TableEditModal(app, column.tableContent ?? '', (value) => {
				callbacks.onTableContentChange(column.name, value);
			}).open();
		});
		...
		renderTableSection(el, column, (content) => callbacks.onTableContentChange(column.name, content));
		return el;
	}
...
// ---- Table section (sectionType 'table') ------------------------------------

interface TableSectionCell {
	text: string;
	cs: number;
	rs: number;
	col: number;
...
function parseTableSectionRows(source: string): string[][] {
	const rows: string[][] = [];
...
function buildMergedTableGrid(rows: string[][]): TableSectionCell[][] {
	const occ: boolean[][] = [];
	const isOcc = (r: number, c: number): boolean => !!occ[r] && !!occ[r][c];
	const setOcc = (r: number, c: number): void => {
		if (!occ[r]) occ[r] = [];
		occ[r][c] = true;
	};
	const grid: TableSectionCell[][] = [];
	rows.forEach((cells, r) => {
		const out: TableSectionCell[] = [];
		let c = 0;
		for (const text of cells) {
			...
			if (text === '^') {
				let above: TableSectionCell | undefined;
				for (let rr = r - 1; rr >= 0 && !above; rr--) {
					above = (grid[rr] ?? []).find(cl => cl.col <= c && c < cl.col + cl.cs);
				}
				if (above) {
					if (above.ext !== r) {
						above.rs++;
					...
					for (let k = 0; k < above.cs; k++) setOcc(r, c + k);
					c += above.cs;
				} else {
					c++;
				}
			...
			const cell: TableSectionCell = { text, cs: 1, rs: 1, col: c };
			setOcc(r, c);
			out.push(cell);
			c++;
		}
...
function tableSectionCellHtml(text: string): string {
	const esc = text
		.replace(/&/g, '&amp;')
		.replace(/</g, '&lt;')
		.replace(/>/g, '&gt;');
...
/** Inverse of buildMergedTableGrid: emit markdown rows with `>` / `^` markers so
 *  in-place cell edits round-trip back into the dashboard file. */
function serializeMergedGrid(grid: TableSectionCell[][]): string {
	let maxCols = 0;
	for (const row of grid) for (const cell of row) maxCols = Math.max(maxCols, cell.col + cell.cs);
	const lines: string[] = [];
	grid.forEach((row, r) => {
		const parts: string[] = [];
		for (let c = 0; c < maxCols; c++) {
			const start = row.find(cl => cl.col === c);
			if (start) {
				parts.push(start.text);
				continue;
			}
			const left = row.find(cl => cl.col < c && c < cl.col + cl.cs);
			if (left) {
				parts.push('>');
				continue;
			}
			...
			for (let rr = r - 1; rr >= 0 && !covered; rr--) {
				const above = (grid[rr] ?? []).find(cl => cl.col <= c && c < cl.col + cl.cs);
				if (above && above.rs > r - rr) {
					parts.push('^');
					covered = true;
				}
			}
			if (!covered) parts.push('');
		}
...
function gridToDenseRows(grid: TableSectionCell[][]): string[][] {
	let maxCols = 0;
	for (const row of grid) for (const cell of row) maxCols = Math.max(maxCols, cell.col + cell.cs);
	const dense: string[][] = [];
	for (let r = 0; r < grid.length; r++) {
		const parts: string[] = [];
		for (let c = 0; c < maxCols; c++) {
			const start = (grid[r] ?? []).find((cl) => cl.col === c);
			if (start) {
				parts.push(start.text);
				continue;
			}
			const left = (grid[r] ?? []).find((cl) => cl.col < c && c < cl.col + cl.cs);
			if (left) {
				parts.push('>');
				continue;
			}
...
function serializeDenseRows(dense: string[][]): string {
	const lines: string[] = [];
	dense.forEach((row, r) => {
		lines.push(`| ${row.join(' | ')} |`);
		if (r === 0) {
			lines.push(`| ${new Array(row.length).fill('---').join(' | ')} |`);
		}
	});
	return lines.join('\n');
}

function addTableRowAt(dense: string[][], r: number, grid: TableSectionCell[][]): string[][] {
	const cols = dense[0]?.length ?? 0;
	const newRow: string[] = [];
	for (let c = 0; c < cols; c++) {
		const cell = (grid[r] ?? []).find((cl) => cl.col <= c && c < cl.col + cl.cs);
		newRow.push(cell && cell.rs > 1 ? '^' : '');
	}
	const next = [...dense];
	next.splice(r + 1, 0, newRow);
	return next;
}

function deleteTableRowAt(dense: string[][], r: number): string[][] {
	if (r <= 1) return dense;
	const next = [...dense];
	const row = next[r];
	if (!row) return dense;
	const below = next[r + 1];
	if (below) {
...
function insertTableColumnAt(dense: string[][], c: number): string[][] {
	const cols = dense[0]?.length ?? 0;
	if (c < 0 || c > cols) return dense;
	return dense.map((row) => {
		let marker = '';
		if (c === cols) {
			const left = c > 0 ? row[c - 1] : undefined;
			if (left === '>' || left === '^') marker = left;
		} else {
			const cur = row[c];
			if (cur === '>' || cur === '^') marker = cur;
		}
		const next = [...row];
		next.splice(c, 0, marker);
		return next;
	});
}

function deleteTableColumnAt(dense: string[][], c: number): string[][] {
	const cols = dense[0]?.length ?? 0;
	if (c < 0 || c >= cols) return dense;
	return dense.map((row) => {
		const removed = row[c];
...
function parseTasks(text: string): TableTask[] {
	if (!text.trim()) return [];
	return text.split(/<br\s*\/?>/i).map((line) => {
		const raw = line.trim();
		const m = raw.match(/^\[([ xX])\]\s*(.*)$/);
		if (m) {
			return { raw, completed: m[1]!.toLowerCase() === 'x', label: m[2]!.trim() };
		}
		return { raw, completed: false, label: raw };
	});
}

function serializeTasks(tasks: TableTask[]): string {
	return tasks
		.map((t) => {
			if (t.completed) return `[x] ${t.label}`;
			return t.label;
		})
		.join('<br>');
}

function renderTaskCell(td: HTMLElement, cell: TableSectionCell, onChange: (value: string) => void): void {
	td.empty();
	td.addClass('apex-table-cell--tasks');
	let tasks = parseTasks(cell.text);

	const renderList = () => {
		td.empty();
		tasks.forEach((task, i) => {
			const row = td.createDiv({ cls: 'apex-table-task-row' });
			const checkbox = row.createEl('input', { cls: 'apex-table-task-check', attr: { type: 'checkbox' } });
			checkbox.checked = task.completed;
			checkbox.addEventListener('change', () => {
				tasks = tasks.map((t, idx) => (idx === i ? { ...t, completed: checkbox.checked } : t));
		...
		addInput.addEventListener('keydown', (ke) => {
			if (ke.key === 'Enter' && addInput.value.trim()) {
				ke.preventDefault();
				ke.stopPropagation();
				tasks = [...tasks, { raw: addInput.value.trim(), completed: false, label: addInput.value.trim() }];
				onChange(serializeTasks(tasks));
			}
		});
...
function renderTableSection(el: HTMLElement, column: DashboardColumn, onEdit: (content: string) => void): void {
	const wrap = el.createDiv({ cls: 'dashboard-table-wrap' });
	const rows = parseTableSectionRows(column.tableContent ?? '');
	if (rows.length === 0) {
		wrap.createDiv({ cls: 'dashboard-library-empty', text: '（空表格：在 dashboard.md 本分区下粘贴 Markdown 表格，支持 > 横并、^ 纵并）' });
		return;
	}
	const grid = buildMergedTableGrid(rows);
	const table = wrap.createEl('table', { cls: 'apex-table' });
	const dense = gridToDenseRows(grid);
		...
		for (const cell of cells) {
			...
			if (isTaskCell) {
				renderTaskCell(td, cell, (value) => {
					cell.text = value;
					onEdit(serializeMergedGrid(grid));
				});
			} else {
				td.innerHTML = tableSectionCellHtml(cell.text);
				td.addClass('apex-table-cell--editable');
			}
			td.addEventListener('contextmenu', (e) => {
				...
				menu.addItem((item) =>
					item
						.setTitle(t('table.addRowBelow'))
						.setIcon('plus')
						.onClick(() => {
							const next = addTableRowAt(dense, r, grid);
							onEdit(serializeDenseRows(next));
						})
				);
				if (r > 1) {
					menu.addItem((item) =>
						item
							.setTitle(t('table.deleteRow'))
							.setIcon('trash-2')
							.onClick(() => {
								const next = deleteTableRowAt(dense, r);
								onEdit(serializeDenseRows(next));
							})
					);
				}
				...
				menu.addItem((item) =>
					item
						...
						.setIcon('align-start')
						.onClick(() => {
							const next = insertTableColumnAt(dense, cell.col);
							onEdit(serializeDenseRows(next));
						})
				);
				menu.addItem((item) =>
					item
						...
						.setIcon('align-end')
						.onClick(() => {
							const next = insertTableColumnAt(dense, cell.col + cell.cs);
							onEdit(serializeDenseRows(next));
						})
				);
				if ((dense[0]?.length ?? 0) > 1) {
					menu.addItem((item) =>
						item
							.setTitle(t('table.deleteColumn'))
							.setIcon('trash-2')
							.onClick(() => {
								const next = deleteTableColumnAt(dense, cell.col);
								onEdit(serializeDenseRows(next));
							})
					);
				}
			...
			td.addEventListener('click', (e) => {
				...
				textarea.value = cell.text.replace(/<br\s*\/?>/gi, '\n');
				textarea.focus();
				textarea.select();
				const resize = () => {
	...
	wrap.createDiv({ cls: 'dashboard-table-hint', text: t('table.hint') });
}
// ---- Table edit modal (sectionType 'table') ---------------------------------

class TableEditModal extends Modal {
	private readonly initial: string;
	private readonly onSave: (value: string) => void;

	constructor(app: App, initial: string, onSave: (value: string) => void) {
		super(app);
	...
	onOpen(): void {
		...
		contentEl.createDiv({ cls: 'dashboard-modal-header' })
			.createDiv({ cls: 'dashboard-modal-title', text: t('table.editTitle') });

		const body = contentEl.createDiv({ cls: 'dashboard-modal-body' });
		const area = body.createEl('textarea', { cls: 'dashboard-table-edit-area' });
		area.value = this.initial;
		area.spellcheck = false;

		const footer = contentEl.createDiv({ cls: 'dashboard-modal-footer' });
		const cancelBtn = footer.createEl('button', { cls: 'mod-normal', text: t('common.cancel') });
		cancelBtn.addEventListener('click', () => this.close());
		const saveBtn = footer.createEl('button', { cls: 'mod-cta', text: t('common.save') });
		saveBtn.addEventListener('click', () => {
